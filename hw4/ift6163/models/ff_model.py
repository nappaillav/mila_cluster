...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # obs_normalized = # DONE(Q1)
        # acs_normalized = # DONE(Q1)
        ...
        # delta_pred_normalized = # DONE(Q1)
        # next_obs_pred = # DONE(Q1)
        delta_pred_normalized = (self.delta_network(concatenated_input))
        next_obs_pred = ptu.from_numpy(obs_unnormalized) + unnormalize(delta_pred_normalized.detach(), delta_mean,delta_std)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        # prediction = # DONE(Q1) get numpy array of the predicted next-states (s_t+1)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        prediction = self.forward(obs,acs,data_statistics['obs_mean'], \
            data_statistics['obs_std'],data_statistics['acs_mean'],data_statistics['acs_std'],\
                data_statistics['delta_mean'],data_statistics['delta_std'])[0]
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        # target = # DONE(Q1) compute the normalized target for the model.
        # # Hint: you should use `data_statistics['delta_mean']` and
        # # `data_statistics['delta_std']`, which keep track of the mean
        # # and standard deviation of the model.

        # loss = # DONE(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        target = self.forward(observations,actions,data_statistics['obs_mean'], \
            data_statistics['obs_std'],data_statistics['acs_mean'],data_statistics['acs_std'],\
                data_statistics['delta_mean'],data_statistics['delta_std'])[1]

        delta_normalized = ptu.from_numpy(normalize(next_observations-observations,data_statistics['delta_mean'],data_statistics['delta_std']))
        loss = self.loss(delta_normalized, target)
