...
class DQNCritic(BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        if self.double_q:
            ...
            qa_next_values = self.q_net(next_ob_no)
            q_tp1  = torch.gather(qa_tp1_values, 1, torch.argmax(qa_next_values, dim=1).unsqueeze(-1)).squeeze(1)
    ...
    def qa_values(self, obs):
        ...
        if self.double_q:
            ...
            qa_values = torch.cat((qa_values, qa_values), dim=1)
            qa_values = qa_values.view(-1,2,self.ac_dim)
        
        # print('---- {} ----'.format(qa_values.shape))
